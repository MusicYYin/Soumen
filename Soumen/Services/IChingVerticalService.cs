...
internal sealed unsafe class IChingVerticalService : IDisposable
{
    ...
    public IChingVerticalService(Configuration configuration, DiagnosticLogger diagnostics)
    {
    ...
    private void Update(IFramework framework)
    {
        ...
        if (normal == null || combat == null)
        {
            try
            {
                var normalAddress = IChingHookAddresses.Resolve("_SendNormalMoveHook", diagnostics);
                var combatAddress = IChingHookAddresses.Resolve("_SendCombatMoveHook", diagnostics);
            ...
            {
                failed = true;
                diagnostics.Write("I-Ching Hook", $"飞天遁地安装失败：{exception.GetType().Name}。");
        ...
        var desired = configuration.IChingVerticalOffset;
    ...
    private nint SendCombat(nuint context, nint data, uint length)
    {
        if (data != 0 && configuration.IChingVerticalMovement)
        {
            ((float*)data)[4] += configuration.IChingVerticalOffset;
            ((float*)data)[7] += configuration.IChingVerticalOffset;
        }
